...
def renumber_pdb(pdb, out_pdb, scheme='imgt', mute=False):
    if pdb.endswith('.pdb'): parser = PDBParser(QUIET=True)
    elif pdb.endswith('.cif'): parser = MMCIFParser(QUIET=True)
    else: raise ValueError(f'file format not supported: {pdb}')
    ...
    for chain in structure.get_chains():
        ...
        for r in list(chain.get_residues()):
            ...
            pos_index += 1
    if out_pdb.endswith('.pdb'): io = PDBIO()
    elif out_pdb.endswith('.cif'): io = MMCIFIO()
    else: raise ValueError(f'Output file format not supported: {out_pdb}')
