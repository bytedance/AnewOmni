...
class ContinuousTransition(nn.Module):
    ...
    def denoise_ddim(self, p_t, eps_p, mask_generate, batch_ids, t, t_next, eta=0.0):
        # IMPORTANT:
        #   clampping alpha is to fix the instability issue at the first step (t=T)
        #   it seems like a problem with the ``improved ddpm''.
        expand_shape = [p_t.shape[0]] + [1 for _ in p_t.shape[1:]]
        ...
        alpha_bar = self.var_sched.alpha_bars[t][batch_ids]
        alpha_bar_next = self.var_sched.alpha_bars[t_next][batch_ids].view(*expand_shape)

        sigma = (eta * self.var_sched.sigmas[t][batch_ids]).view(*expand_shape)
        c = torch.sqrt(1 - alpha_bar_next - sigma ** 2)
        ...
        p_next = c0 * (p_t - c1 * eps_p) + c * eps_p + sigma * z
        p_next = torch.where(mask_generate.expand_as(p_t), p_next, p_t)
        return p_next
